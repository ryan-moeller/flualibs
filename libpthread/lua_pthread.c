...
#define DEFINE(ident) ({ \
	...
	DEFINE(DETACHED);
	DEFINE(SCOPE_SYSTEM);
	DEFINE(INHERIT_SCHED);
	DEFINE(NOFLOAT);

	DEFINE(CREATE_DETACHED);
	DEFINE(CREATE_JOINABLE);
	DEFINE(SCOPE_PROCESS);
	DEFINE(EXPLICIT_SCHED);

	DEFINE(PROCESS_PRIVATE);
	DEFINE(PROCESS_SHARED);

	DEFINE(CANCEL_ENABLE);
	DEFINE(CANCEL_DISABLE);
	DEFINE(CANCEL_DEFERRED);
	DEFINE(CANCEL_ASYNCHRONOUS);
